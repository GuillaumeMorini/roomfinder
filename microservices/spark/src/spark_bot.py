...
'''
    ...
    export roomfinder_spark_bot_email=toto@sparkbot.io

    Find Cisco Spark developer token on http://developer.ciscospark.com
    export spark_token=...

    # Address for the Spark Bot itself
...
import requests, json, re, random, socket
import ntpath
import datetime
...
def stats(user,roomid):
    logfile = open(log_dir+"ILM-RoomFinder-Bot.log", 'a+')
...
def optout(user):
    f = open(log_dir+"ILM-RoomFinder-Bot.log","a+")
    d = f.readlines()
    f.seek(0)
    for i in d:
...
def process_webhook():
    ...
    reply=None
    removed = False
    processing_message="I am processing your request"
    ...
    #     sys.stderr.write("markdown: "+str(message["markdown"])+"\n")
    # if "html" in message:
    #     sys.stderr.write("html: "+str(message["html"])+"\n")
    text=message["text"].lstrip().rstrip()
    ...
    sys.stderr.write("Incoming room message: %s\n" % text )
    log(post_data['data']['personEmail']+" - " +post_data['data']['roomId'],str(text),processing_message)
    send_message_to_room(post_data["data"]["roomId"], processing_message,message_type)
    log_message_to_room(log_room_id, post_data['data']['personEmail'], str(text), processing_message,message_type)
...
def find_dir(cco):
    ...
    ############
    try:
        page = requests.post(URL, headers = app_headers, json=data)
    except requests.exceptions.RequestException:
        return "Issue with directory connectivity"
    text=page.text
    sys.stderr.write('find_dir_reply: '+str(text))
    if text == None or text == "" or text == "{}" :
        return "This directory entry was not found !"
    else:
        reply=json.loads(text.replace("\'","\""))
        if len(reply) == 1 :
            key=list(reply.keys())[0]
            return reply[key]
        else:
            text="Are you looking for one of these people:"
...
def find_image(keyword):
    ...
    test=page.text.replace('jsonFlickrFeed(','').replace(')','').replace('\\\'','\\\\\'')
    j=json.loads(test)
    if len(j["items"]) > 0 :
...
def post_localfile(roomId, photo_url, text='', html='', markdown='', toPersonId='', toPersonEmail=''):
...
def log_message_to_room(room_id, author, message, message_reply,message_type="text"):
    ...
    if message_type == "text":
        ...
        title=message_reply["title"]
        manager=message_reply["manager_name"]
        phone=message_reply["telephonenumber"]
        mobile=message_reply["mobile"]
        photo=message_reply["pic"]
        dir_url=message_reply["link"]
        country=message_reply["co"]
        building=message_reply["building"]
        tmp="Author: "+author+" <br>\n Request: "+message+" <br>\n Reply: <br>\n "

        if name!= "":
            tmp+="<b>Name</b>: "+str(name)+'<br>'
        if title != "":
            tmp+='<b>Title</b>: '+str(title)+'<br>'
        if manager != "":
            tmp+='<b>Manager</b>: '+str(manager)+'<br>'
        if country != "":
            tmp+='<b>Country</b>: '+str(country)+'<br>'
        if building != "":
            tmp+='<b>Building</b>: '+str(building)+'<br>'
        if phone != "":
            tmp+='<b>Work</b>: '+str(phone)+'<br>'
        sys.stderr.write('Mobile: '+str(mobile)+"\n")
        if mobile != "":
            tmp+='<b>Mobile</b>: '+str(mobile)+'<br>'
        if dir_url != "":
            tmp+="<b>Internal directory</b>: <a href=\""+dir_url+"\">link</a>"
...
def send_message_to_room(room_id, message,message_type="text"):
    ...
    if message_type == "text":
        ...
        title=message["title"]
        manager=message["manager_name"]
        country=message["co"]
        phone=message["telephonenumber"]
        mobile=message["mobile"]
        photo=message["pic"]
        dir_url=message["link"]
        ...
        with open(filename, 'wb') as handle:
            handle.write(requests.get(photo).content)    
        return post_localfile(room_id,photo,html=tmp)
...
if __name__ == '__main__':
    ...
    if (bot_url == None):
        ...
        if (bot_url == None):
            sys.stderr.write("Spark Bot URL not defined !! Exiting !!")
            exit(1)
    ...
    if (bot_email == None):
        ...
        if (bot_email == None):
            sys.stderr.write("Spark Bot email not defined !! Exiting !!")
            exit(1)
    ...
    if (spark_token == None):
        ...
        if (spark_token == None):
            sys.stderr.write("Spark Bot token not defined !! Exiting !!")
            exit(1)
    ...
    if (log_room_id == None):
        ...
        if (log_room_id == None):
            get_log_room_id = input("What is the Cisco Spark Log Room ID? ")
